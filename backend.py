...
reports_collection = db.get_collection("reports")  # Collection for report history
...
def signup():
    ...
    # Save user details to MongoDB (plain text as requested; consider hashing in production)
...
def analyze():
    try:
        ...
        email = request.form.get('email')  # Retrieve the user's email from form data
...
"""
        ...
        # Generate the analysis using Gemini AI
        response = client.models.generate_content(
            model="gemini-2.0-flash",
            contents=prompt
        )

        # Calculate the timestamp once and reuse it
        now = datetime.now()
        current_date = now.strftime("%Y-%m-%d %H:%M:%S")
        header = f"Report generated on {current_date}\n"
        full_report = header + response.text

        # Save the report to the MongoDB reports collection, including the email
